...
export const edviceDocsfileUploadApi = async (file: File) => {
    try {
        ...
        const response = await axios.post(`${backendUrl}/api/v1/organiser/organiser-approval-edvince-upload`, formData, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'multipart/form-data'
            }
...
export const requestApprovalApi = async (schoolName: string, locality: string, role: string, evidenceDocs: string) => {
    try {
        const token = localStorage.getItem('token');
        const response = await axios.post(`${backendUrl}/api/v1/organiser/organiser-approval-request`, {
            schoolName,
            locality,
            role,
            evidenceDocs
...
export const eventImagefileUploadApi = async (file: File) => {
    try {
        ...
        const formData = new FormData();
        formData.append('file', file);
        const response = await axios.post(`${backendUrl}/api/v1/event/create-event-images-upload`, formData, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'multipart/form-data'
            }
...
export const createEventApi = async (eventName: string,locality: string,school: string,coverImage: string,eventLogo: string,eventDescription: string,eventStartDate: string,eventEndDate: string,numberOfSeats: string,feesPerDelegate: string,totalRevenue: string,website: string,instagram: string) => {
    try {
        const token = localStorage.getItem('token');
        const response = await axios.post(`${backendUrl}/api/v1/event/create-event`, {
            eventName,
            locality,
            school,
            coverImage,
