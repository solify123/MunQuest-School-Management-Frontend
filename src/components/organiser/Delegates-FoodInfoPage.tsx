...
const DelegatesFoodInfoPage: React.FC = () => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                toast.error('Failed to load food info');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const foods: DelegateFoodItem[] = useMemo(() => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                {/* Header Row */}
                <div className="grid gap-2 mb-2" style={{ gridTemplateColumns: '100px 180px 150px 250px 150px 150px' }}>
                    {['Unique ID', 'Name', 'Mobile', 'Email', 'Food', 'Allergies'].map((header) => (
                        header === ' ' ? (
                ...
                {/* Data Rows */}
                {filtered.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">{searchTerm ? 'No delegates found matching your search' : 'No delegate food info found'}</div>
                ) : (
                    filtered.map((row) => (
                        <div key={row.id} className="grid gap-2 mb-2" style={{ gridTemplateColumns: '100px 180px 150px 250px 150px 150px' }}>
                            ...
                            <div className="bg-white px-3 py-2 text-sm text-gray-900 rounded-md border border-gray-200">{row.food || '-'}</div>
                            <div className="bg-white px-3 py-2 text-sm text-gray-900 rounded-md border border-gray-200">{row.allergies || '-'}</div>
                        </div>
                    ))
                )}
            </div>
...
export default DelegatesFoodInfoPage;
